...
* You have different Databricks workspace environments (e.g. a "test" workspace for CI, in addition to dev_qa/uat/prod)
...
### Fork or customize this repository
You can customize the MLOps Stacks in this repo directly, or fork it. You may want to keep a fork private if you're tailoring
the stack to the specific needs of your organization, or public if you're creating
a generic new stack.
...
Update parameters in your fork as needed in `mlops-stacks/databricks_template_schema.json` and update corresponding template variable in `mlops-stacks/library/template_variables.tmpl`. Pruning the set of
parameters makes it easier for data scientists to start new projects, at the cost of reduced flexibility.

For example, you may have a fixed set of uat & prod Databricks workspaces (or use a single uat & prod workspace), so the
`input_databricks_uat_workspace_host` and `input_databricks_prod_workspace_host` parameters may be unnecessary. 
...
``mlops-stacks/template/{{.input_root_dir}}/{{template `project_name_alphanumeric_underscore` .}}/resources``, along with docs.
