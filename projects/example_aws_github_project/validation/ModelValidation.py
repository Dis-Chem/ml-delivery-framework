...
#
# It runs as part of CD and by an automated model training job -> validation -> deployment job defined under ``example_aws_github_project/resources/model-workflow-resource.yml``
#
#
# Parameters:
#
...
#
# Custom metrics, validation thresholds, and evaluator config are defined in validation/validation.py.
#
...
dbutils.widgets.text(
    "experiment_name",
    "/dev-example-aws-github-project-experiment",
    "Experiment Name",
)
...
dbutils.widgets.text("model_name", "dev.test_project_schema_name.example-aws-github-project-model", "Full (Three-Level) Model Name")
...
from validation import custom_metrics as _custom_metrics, validation_thresholds as _validation_thresholds, evaluator_config as _evaluator_config
...
def get_fs_model(df):
    ...
    return (
        fe_client.score_batch(model_uri=model_uri, df=spark.createDataFrame(df))
        .select("prediction")
        .toPandas()
    )
