...
The databricks CLI bundles top level is defined by file `example_aws_github_project/databricks.yml`.
...
ML Resource Configurations in this directory:
 - model workflow (`example_aws_github_project/resources/model-workflow-resource.yml`)
 - batch inference workflow (`example_aws_github_project/resources/batch-inference-workflow-resource.yml`)
 - monitoring resource and workflow (`example_aws_github_project/resources/monitoring-resource.yml`)
 - feature engineering workflow (`example_aws_github_project/resources/feature-engineering-workflow-resource.yml`)
 - model definition and experiment definition (`example_aws_github_project/resources/ml-artifacts-resource.yml`)
...
If you want to use different catalog names, please update the `targets` declared in the `example_aws_github_project/databricks.yml` and `example_aws_github_project/resources/ml-artifacts-resource.yml` files.
...
|-------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------------------|-------------------------------------|------------------------------------------------|
| dev         | The `dev` deployment target is used by ML engineers to deploy ML resources to development workspace with `dev` configs. The config is for ML project development purposes.                                                           | dev workspace        | dev-example-aws-github-project-model     | /dev-example-aws-github-project-experiment     |
| staging     | The `staging` deployment target is part of the CD pipeline. Latest main content will be deployed to staging workspace with `staging` config.                                                             | staging workspace    | staging-example-aws-github-project-model | /staging-example-aws-github-project-experiment |
| prod        | The `prod` deployment target is part of the CD pipeline. Latest release content will be deployed to prod workspace with `prod` config.                                                                      | prod workspace       | prod-example-aws-github-project-model    | /prod-example-aws-github-project-experiment    |
| test        | The `test` deployment target is part of the CI pipeline. For changes targeting the main branch, upon making a PR, an integration test will be triggered and ML resources deployed to the staging workspace defined under `test` deployment target. | staging workspace    | test-example-aws-github-project-model    | /test-example-aws-github-project-experiment    |
...
2. Complete the `TODO` in `example_aws_github_project/databricks.yml` to add the dev workspace URI under `targets.dev.workspace.host`.
...
```
Following this, update the batch_inference_job base parameters in `example_aws_github_project/resources/batch-inference-workflow-resource.yml` to pass
...
To get started, open `example_aws_github_project/resources/batch-inference-workflow-resource.yml`.  The file contains the ML resource definition of a batch inference job, like:

```$xslt
resources:
  jobs:
    batch_inference_job:
      name: ${bundle.target}-example-aws-github-project-batch-inference-job
      tasks:
...
The example above defines a Databricks job with name `${bundle.target}-example-aws-github-project-batch-inference-job`
that runs the notebook under `example_aws_github_project/deployment/batch_inference/BatchInference.py` to regularly apply your ML model for batch inference.

Jobs run on [serverless compute](https://docs.databricks.com/jobs/run-serverless-jobs.html) by default: the task references a serverless environment by its `environment_key` (`default` here), and that key is defined once under `environments`, with its own dependencies installed from `requirements.txt`. Multiple tasks in the same job can share the same `environment_key` to reuse one environment definition.

We specify a `batch_inference_job` under `resources/jobs` to define a databricks workflow with internal key `batch_inference_job` and job name `{bundle.target}-example-aws-github-project-batch-inference-job`.
...
`test-example-aws-github-project-batch-inference-job`. During the deployment of the `staging` environment config, the job name will be
`staging-example-aws-github-project-batch-inference-job`.
